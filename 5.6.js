...
function filtrarStreamers(palabra) {
  const resultados = streamers.filter((streamer) =>
    streamer.name.toLowerCase().includes(palabra.toLowerCase())
  )
  return resultados
}

// Ejemplo de uso
const palabra = prompt('Introduce una palabra para filtrar streamers:')
const streamersFiltrados = filtrarStreamers(palabra)

console.log('Streamers filtrados:', streamersFiltrados)
